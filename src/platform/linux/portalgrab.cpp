...
namespace portal {
  ...
  private:
    ...
    static void create_request_path(GDBusConnection *conn, gchar **out_path, gchar **out_token) {
      ...
      if (out_token) {
        *out_token = g_strdup_printf("Plank%u", request_count);
      }
      if (out_path) {
        g_autofree gchar *sender = get_sender_string(conn);
        *out_path = g_strdup(std::format("{}{}{}{}", REQUEST_PREFIX, sender, "/Plank", request_count).c_str());
      }
    }
    ...
    static void create_session_path(GDBusConnection *conn, gchar **out_path, gchar **out_token) {
      ...
      if (out_token) {
        *out_token = g_strdup_printf("Plank%u", session_count);
      }

      if (out_path) {
        g_autofree gchar *sender = get_sender_string(conn);
        *out_path = g_strdup(std::format("{}{}{}{}", SESSION_PREFIX, sender, "/Plank", session_count).c_str());
      }
    }
